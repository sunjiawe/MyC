...
Status
SubString(String Sub,String S,int pos,int len)
{
    int i;
...
Status
StrInsert(String S,int pos,String T)
{
    int i;
    if(pos<1||pos>S[0]+1)
        return ERROR;
...
Status
StrDelete(String S,int pos,int len)
{
    int i;
...
{
    int i=1;/*从串S的第一个字符起查找串T*/if(StrEmpty(T))/*T是空串*/
        return ERROR;
    do
        {
